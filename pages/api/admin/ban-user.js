/**
 * @fileoverview Admin Ban/Unban User API
 * Disables or enables a user's auth account
...
export default async function handler(req, res) {
    ...
    if (typeof ban !== 'boolean') return res.status(HTTP_STATUS.BAD_REQUEST).json({ error: 'ban must be boolean' });

    // Prevent self-ban
    if (userId === auth.userId) {
        return res.status(HTTP_STATUS.BAD_REQUEST).json({ error: 'Cannot ban your own account' });
    }

    try {
        // Supabase uses ban_duration to ban users
        // 'none' unbans, a large duration effectively bans
