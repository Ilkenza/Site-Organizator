    ...
    for (let i = 1; i < allRows.length; i++) {
        ...
        if (!site.name) {
            try {
                ...
                site.name = urlObj.hostname.replace(/^www\./, '');
            } catch {
                site.name = site.url;
            }
        }
...
export async function importSites(sites, userId, onProgress = null, options = {}) {
    ...
    try {
        ...
        const mergedReport = { created: [], updated: [], skipped: [], errors: [] };
        ...
        // Save import source to localStorage for icon display (no tag needed)
        if (importSource && typeof window !== 'undefined') {
            try {
                const existing = JSON.parse(localStorage.getItem('import_sources') || '{}');
                const importedUrls = [
                    ...(mergedReport.created || []).map(r => r.site?.url),
                    ...(mergedReport.updated || []).map(r => r.site?.url)
                ].filter(Boolean);
                importedUrls.forEach(url => { existing[url] = importSource; });
                localStorage.setItem('import_sources', JSON.stringify(existing));
            } catch { }
        }
