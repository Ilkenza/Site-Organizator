...
// includes: array like ['sites', 'categories', 'tags']
function generateFilename(format, includes) {
    const timestamp = new Date().toISOString().split('T')[0];
    const ext = FILE_EXTENSIONS[format] || 'json';
    const prefix = Array.isArray(includes) && includes.length > 0
        ? includes.join('-')
        : 'sites-export';
...
// includes: array of what to include, e.g. ['sites', 'categories', 'tags']
export async function exportSites(userId, format = 'json', includes = ['sites', 'categories', 'tags', 'groups']) {
    try {
        ...
        const includeParam = Array.isArray(includes) && includes.length > 0 ? `&include=${includes.join(',')}` : '';
        const response = await fetch(`/api/export?format=${format}${includeParam}`, { headers });

        if (!response.ok) {
            throw new Error(`Export failed: ${response.statusText}`);
        }

        const filename = generateFilename(format, includes);
...
export async function parseImportFile(file, source = 'auto') {
    ...
    try {
        ...
        if (filename.endsWith('.json')) {
            ...
            // Preserve groups if present in the export
            if (json.groups) result.groups = json.groups;
...
function parseCSV(csvText) {
    ...
    for (let i = 1; i < allRows.length; i++) {
        ...
        headers.forEach((header, index) => {
            ...
            // Use Case
            else if (header === 'usecase' || header === 'use_case') {
                site.use_case = value;
            }
            ...
            // Used On
            else if (header === 'usedon' || header === 'used_on') {
                const v = value.toLowerCase();
                if (['desktop', 'mobile', 'both', 'web'].includes(v)) {
                    site.used_on = v;
                }
            }
        });
...
function convertSiteToRow(site) {
    ...
    const row = {
        ...
        is_needed: site.is_needed !== undefined ? site.is_needed : null,
    ...
    // Preserve per-site merge mode flag
    if (site.mergeMode !== undefined) {
        row.mergeMode = site.mergeMode;
    }
...
// Sends rows in chunks to avoid serverless function timeouts (Vercel ~10-60s limit).
...
export async function importSites(sites, userId, onProgress = null, options = {}) {
    const { useFoldersAsCategories = true, importSource = null, signal = null, groups = null, mergeMode = false, perSiteMerge = false } = options;
    ...
    try {
        ...
        const mergedReport = { created: [], updated: [], skipped: [], errors: [], categoriesCreated: 0, tagsCreated: 0, groupsImported: 0 };
            ...
            try {
                const result = await fetchAPI('/import', {
                    ...
                    body: JSON.stringify({
                        ...
                        perSiteMerge: perSiteMerge || undefined,
                        // Send groups only with the first chunk
                        ...(c === 0 && groups ? { groups } : {})
                    }),
                ...
                if (report.groupsImported) mergedReport.groupsImported = report.groupsImported;
