...
function matchesPattern(text, pattern) {
    // Patterns containing dots (like 'fonts.google', 'fly.io') — use direct substring match
    if (pattern.includes('.')) {
...
export function suggestCategories(url, name = '', existingCategories = []) {
    ...
    // Reverse matching: check if any existing category name appears in the URL or title
    // This catches user-created categories like "React", "Fonts", "AI Tools", etc.
    if (existingCategories.length > 0) {
        ...
        for (const cat of existingCategories) {
            ...
            // Check each word in multi-word category names (e.g. "Web Development" → check "web", "development")
            const words = catName.split(/\s+/);
            // For multi-word names, also check the full name as a phrase
            const patternsToCheck = words.length > 1 ? [catName, ...words.filter(w => w.length > 2)] : [catName];
