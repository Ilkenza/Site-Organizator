    ...
    return (
        ...
        `}
            >
                ...
                {/* Usage Filter for Categories tab */}
                {activeTab === 'categories' && (
                    <div className="p-3 sm:p-4">
                        ...
                        <div className="space-y-1">
                            {[{ key: 'all', label: 'All Categories' }, { key: 'used', label: 'Used in Sites' }, { key: 'unused', label: 'Not Used' }].map(f => (
                                <button
                                    key={f.key}
                                    onClick={() => setUsageFilterCategories(f.key)}
                                    className={`w-full px-3 py-2 rounded-lg text-xs font-medium transition-all flex items-center justify-between ${usageFilterCategories === f.key
                                        ? 'bg-app-accent/10 text-app-accent border border-app-accent/30'
                                        : 'text-app-text-secondary hover:bg-app-bg-light border border-transparent'
                                        }`}
                                >
                        ...
                        {usageFilterCategories !== 'all' && (
                            <button
                                onClick={() => setUsageFilterCategories('all')}
                ...
                {activeTab === 'tags' && (
                    <div className="p-3 sm:p-4">
                        ...
                        <div className="space-y-1">
                            {[{ key: 'all', label: 'All Tags' }, { key: 'used', label: 'Used in Sites' }, { key: 'unused', label: 'Not Used' }].map(f => (
                                <button
                                    key={f.key}
                                    onClick={() => setUsageFilterTags(f.key)}
                                    className={`w-full px-3 py-2 rounded-lg text-xs font-medium transition-all flex items-center justify-between ${usageFilterTags === f.key
                                        ? 'bg-app-accent/10 text-app-accent border border-app-accent/30'
                                        : 'text-app-text-secondary hover:bg-app-bg-light border border-transparent'
                                        }`}
                                >
                        ...
                        {usageFilterTags !== 'all' && (
                            <button
                                onClick={() => setUsageFilterTags('all')}
                                className="w-full mt-2 px-3 py-1.5 rounded-lg text-xs font-medium transition-all flex items-center justify-center gap-1 bg-red-500/10 text-red-400 border border-red-500/30 hover:bg-red-500/20"
                            >
                                <CloseIcon className="w-3 h-3" />
                                Reset Filter
