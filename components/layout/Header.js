...
export default function Header({ onAddClick, onMenuClick }) {
    ...
    // Keyboard shortcuts: Ctrl/Cmd+K to focus search, Ctrl/Cmd+N to open Add Site modal
    useEffect(() => {
        const handler = (e) => {
            ...
            if (mod && key === 'n') {
                e.preventDefault();
                window.dispatchEvent(new Event('openAddSiteModal'));
            }
        };
                    ...
                    <div className="flex-1 min-w-0 max-w-xs sm:max-w-md">
                        <div className="relative">
                            ...
                            <input
                                ...
                                placeholder={
                                    activeTab === 'sites'
                                        ? 'Search sites... (Ctrl/Cmd+K)'
                                        : activeTab === 'categories'
                                            ? 'Search categories... (Ctrl/Cmd+K)'
                                            : activeTab === 'tags'
                                                ? 'Search tags... (Ctrl/Cmd+K)'
                                                : 'Search... (Ctrl/Cmd+K)'
                                }
                                value={searchQuery}
